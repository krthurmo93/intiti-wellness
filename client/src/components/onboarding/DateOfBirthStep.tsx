...
export function DateOfBirthStep({ value, onChange, onNext, onBack }: DateOfBirthStepProps) {
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          >
            <Input
              ...
              className="h-14 text-lg text-center border-2 border-stone-200 rounded-xl bg-white/80 focus:border-amber-400 focus:ring-amber-400/20"
              data-testid="input-date-of-birth"
            />
            {touched && !isValid() && (
              <p className="text-sm text-red-500 mt-2 text-center">Please enter a valid birth date</p>
            )}
          </motion.div>
          ...
          {[0, 1, 2, 3, 4, 5, 6].map((i) => (
            <div
              ...
              className={`w-2 h-2 rounded-full transition-all ${
                i === 3 ? "bg-amber-500 w-6" : "bg-stone-300"
              }`}
            />
          ))}
        </div>
